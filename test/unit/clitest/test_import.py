...
class TestImport(object):
    ...
    def mkfakescreen(
        self, screen_dir, nplates=2, nruns=2, nwells=2, nfields=4, with_ds_store=False
    ):

        fieldfiles = []
        for iplate in range(nplates):
            plate_dir = self.mkdir(
                screen_dir, "Plate00%s" % str(iplate), with_ds_store=with_ds_store
            )
            for irun in range(nruns):
                run_dir = self.mkdir(
                    plate_dir, "Run00%s" % str(irun), with_ds_store=with_ds_store
                )
                for iwell in range(nwells):
                    well_dir = self.mkdir(
                        run_dir, "WellA00%s" % str(iwell), with_ds_store=with_ds_store
                    )
                    for ifield in range(nfields):
                        fieldfile = old_div(well_dir, ("Field00%s.fake" % str(ifield)))
                        fieldfile.write("")
    ...
    def mkfakepattern(self, tmpdir, nangles=7, ntimepoints=10):
        ...
        for angle in range(1, nangles + 1):
            for timepoint in range(1, ntimepoints + 1):
                tiffile = old_div(
                    spim_dir, ("spim_TL%s_Angle%s.fake" % (str(timepoint), str(angle)))
                )
                tiffile.write("")
                print(str(tiffile))
    ...
    def testLogPrefix(self, tmpdir, capfd):
        ...
        self.add_client_dir()
        self.args += ["-f", "---logprefix=%s" % prefix, "---file=out", "---errs=errs"]
        self.args += [str(fakefile)]
        self.cli.invoke(self.args, strict=True)
    ...
    def testBulkJavaArgs(self):
        ...
        class MockImportControl(ImportControl):
            def do_import(self, command_args, xargs, mode):
                assert "--checksum-algorithm=File-Size-64" in command_args.java_args()
        ...
        class MockImportControl(ImportControl):
            def do_import(self, command_args, xargs, mode):
                if skip in ["all", "checksum"]:
                    assert (
                        "--checksum-algorithm=File-Size-64" in command_args.java_args()
                    )
